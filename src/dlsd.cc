...
void ComputeClosestPoint(double x, double y, const double *params, const Eigen::Vector3d &axis, double &xOut, double &yOut) {
  Eigen::Vector3d p;
...
// TODO Description
template <typename CameraModel>
void ValidatePolygonalChain(const double *x, const double *y, int segmentNo, const double *params, const std::vector<PolygonalChain> &chains, const std::vector<LineSegment> &lines, std::vector<DistortedLineSegment> &distortedLineSegments, int min_single_line_len = MIN_SINGLE_LINE_LENGTH) {
  // Validation
  // Single lines (i.e. not assigned to a cluster) can be accepted without validation
  ...
  for (const PolygonalChain &chain : chains) {
    ...
    for (int index = 0; index < noLines; index++) {
      ...
      if (noLines > 1) {
        for (int k = 0; k < line.len; k++) {
          Eigen::Vector3d v;
          CameraModel::ImageToWorld(params, x[line.firstPixelIndex + k], y[line.firstPixelIndex + k], &v.x(), &v.y());
          v.z() = 1.;
    ...
    for (int line_index = 0; line_index <= noLines; line_index++) {
      ...
      for (int k = 0; k < gap.len; k++) {
        ...
        double _x, _y;
        ComputeClosestPoint<CameraModel>(x[index], y[index], params, axis, _x, _y);
        ...
        if (line_index == 0) { // pregap
          ...
          if (d > REPROJECTION_TH) distortedLineSegment.invalidPixels.insert(k - firstPixelIndex);
          else distortedLineSegment.segment.emplace_back(cv::Point(x[index], y[index]));
        }
      }
...
void dlsd(const cv::Mat &image, const double *params, std::vector<DistortedLineSegment> &distortedLineSegments) {
  ...
  ED ed = ED(image, SOBEL_OPERATOR, 36, 8, 1, 10, 1.0, true); // apply ED algorithm
  ...
  for (const std::vector<cv::Point> &segment : ed.getSortedSegments()) {
    ...
    ValidatePolygonalChain<CameraModel>(x, y, segmentNo, params, chains, lines, distortedLineSegments);
...
template void dlsd<FOVCameraModel>(const cv::Mat &image, const double *params, std::vector<DistortedLineSegment> &distortedLineSegments);
